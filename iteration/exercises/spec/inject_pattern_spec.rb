...
RSpec.describe 'inject pattern test' do
  ...
  it 'test 6' do
    ...
    toppings = []
    topping_calories.each do |topping, _calorie|
  ...
  it 'test 8' do
    ...
    total_calories = 0
    toppings.each do |topping, nutritional_content|
      total_calories += (nutritional_content[:calories] * nutritional_content[:quantity])
    end
  ...
  it 'test 9' do
    ...
    final_grade = 0.0
    grades.each do |topic, grades_array|
      final_grade += grades_array.sum / grades_array.size.to_f
    end
  ...
  it 'test 10' do
    ...
    printable_menu = "Menu:\n"
    menu.each do |food, ingredients|
      gluten = ""

      if ingredients[:gluten_free] == true
        gluten = "(gluten free)"
      else
        gluten = "(non gluten free)"
      end

      printable_menu += "- " + ingredients[:flavors].join(", ") + " " + food.to_s + " " + gluten + "\n"
    end

    expected =  "Menu:\n"\
                "- chicken, potato, steak, veggie empanadas (non gluten free)\n"\
                "- blueberry, vanilla scones (non gluten free)\n"\
